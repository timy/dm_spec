...
void repr_set_exciton_dimer( parameters *ps, para_repr_dimer_local *pdl )
{
    ...
    for (int i_dim = 0; i_dim < ps->n_dim; i_dim ++) {
        // mu_10
        ps->dipole[0][i_dim] = 
            - s_theta * pdl->mu_A[i_dim] + c_theta * pdl->mu_B[i_dim];
        // mu_20
        ps->dipole[1][i_dim] = 
            + c_theta * pdl->mu_A[i_dim] + s_theta * pdl->mu_B[i_dim];
        // mu_31
        ps->dipole[2][i_dim] = 
            - s_theta * pdl->mu_B[i_dim] + c_theta * pdl->mu_A[i_dim];
        // mu_32
        ps->dipole[3][i_dim] = 
            + c_theta * pdl->mu_B[i_dim] + s_theta * pdl->mu_A[i_dim];
    }
...
void para_repr_dimer_config( config_t *cfg, para_repr_dimer_local *pdl )
{
...
#ifndef USE_MPI
    printf( "@para_repr_dimer_config\n" );
    print( pdl->EA );
    print( pdl->EB );
#endif
    pdl->EA *= C_cm2au; pdl->EB *= C_cm2au;
    ...
    for (int i_dim = 0; i_dim < 3; i_dim ++)
        pdl->mu_B[i_dim] =
            config_setting_get_float_elem( setting_2, i_dim );
    // pos
    setting_1 = config_lookup( cfg, "position" );
    assert( config_setting_length( setting_1 ) == 3 );
    for (int i_dim = 0; i_dim < 3; i_dim ++)
        pdl->pos[i_dim] =
            config_setting_get_float_elem( setting_1, i_dim );
}
